...
GeneralPage::GeneralPage(EditorWindow* _parent) : wxPanel(_parent), parent(_parent) {
    sizer = new wxStaticBoxSizer(wxVERTICAL, this);
    SetSizer(sizer);

    sizer->Add(boardSection(sizer), BOXITEMFLAGS);
    sizer->Add(optionSection(sizer), BOXITEMFLAGS);
